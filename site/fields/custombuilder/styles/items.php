...
<?php
  ...
  ( $entry->x() ) ? $itemStyles .= 'top: '.$entry->x().'%; ' : '';
  ( $entry->y() ) ? $itemStyles .= 'left: '.$entry->y().'%; ' : '';
  ( $entry->width() ) ? $itemStyles .= 'width: '.$entry->width().'px; ' : '';
  ( $entry->height() ) ? $itemStyles .= 'height: '.$entry->height().'px; ' : '';

  $itemStyles .= 'position: absolute;';
?>

<div class="builder-item-wrapper" style="<?= $itemStyles ?>">
<div
  class="structure-entry structure-entry-<?php echo strtolower($entry->_fieldset()) ?> builder-entry builder-entry-<?php echo strtolower($entry->_fieldset()) ?>"
  id="structure-entry-<?php echo $entry->id() ?>">

  <div class="structure-entry-content builder-entry-content text">
    <?php if($field->fieldsets() && isset($field->fieldsets()[$entry->_fieldset()])) : ?>
      <label class="builder-entry-fieldset-name">
