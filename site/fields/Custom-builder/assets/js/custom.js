(function($) {
  ...
  var Builder = function(element) {
    ...
    var draggableEntries = element.find('.builder-item-wrapper');
    var parentSizes = {
    ...
    var dragConfig = {
        stop: function(event, el) {
                var left = el.position.left / parentSizes.width * 100;
                var top = el.position.top / parentSizes.height * 100;
                var formUrl = el.helper.find('.structure-edit-button').attr('data-href')
                //el.helper.closest('form').trigger('keep');
                $.get( formUrl, function( data ) {
                  var newData = {};

                  $(data.content).find('form').serializeArray().forEach(function(e, i ){
                  ...
                  $.post( formUrl, newData )
                })
                el.helper.css({top: top + '%', left: left + '%'});
        }
    }

    var resizeConfig = {
      resize: function(event, el){
        var width = $(el.element).width();
        var height = $(el.element).height();
        console.log(height)
        //el.helper.find('.builder-item').css({width: width + 'px', height: height + 'px'});
      },
        stop: function(event, el) {
                var width = el.helper.width();
                var height = el.helper.height();
                var formUrl = el.helper.find('.structure-edit-button').attr('data-href')
                $.get( formUrl, function( data ) {
                  var newData = {};
                  ...
                  $(data.content).find('form').serializeArray().forEach(function(e, i ){
                    ...
                    switch ( e.name ) {
                      case 'width':
                         newData[e.name] = width;
                        break;
                      case 'height':
                         newData[e.name] = height;
                        break;
                      default:
                        break;
                    }
                ...
                el.helper.find('.builder-item').css({width: width + 'px', height: height + 'px'});
                el.helper.css({width: width + 'px', height: height + 'px'});
        }
    }

    draggableEntries.resizable(resizeConfig);

    draggableEntries.draggable(dragConfig)
